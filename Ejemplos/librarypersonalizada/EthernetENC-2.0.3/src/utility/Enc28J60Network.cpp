...
bool Enc28J60Network::init(uint8_t* macaddr)
{
  ...
  RED.beginTransaction(SPI_ETHERNET_SETTINGS);
...
uint8_t Enc28J60Network::readByte(uint16_t addr)
{
  ...
  CSACTIVE;
  // issue read command
  RED.transfer(ENC28J60_READ_BUF_MEM);
  // read data
  uint8_t c = RED.transfer(0x00);
  CSPASSIVE;
  return c;
}
...
void Enc28J60Network::writeByte(uint16_t addr, uint8_t data)
{
  ...
  CSACTIVE;
  // issue write command
  RED.transfer(ENC28J60_WRITE_BUF_MEM);
  // write data
  RED.transfer(data);
  CSPASSIVE;
}
...
Enc28J60Network::readOp(uint8_t op, uint8_t address)
{
  CSACTIVE;
  // issue read command
  RED.transfer(op | (address & ADDR_MASK));
  // read data
  if(address & 0x80)
    {
